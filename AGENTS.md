...
- Use Go 1.24 from [go.mod](go.mod). The CLI behavior lives in [cmd/pgi/main.go](cmd/pgi/main.go).
- Treat [pgi](pgi) as the root shell wrapper. It just `go run ./cmd/pgi` from the repository root.
...
- For build or release changes, use `go build -trimpath -ldflags="-s -w" -o pgi ./cmd/pgi` as the baseline and keep [install.sh](install.sh), [README.md](README.md), and [.github/workflows/release.yml](.github/workflows/release.yml) aligned.
